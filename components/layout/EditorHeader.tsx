...
export function EditorHeader({
  ...
  return (
    <header className="shrink-0 border-b border-[var(--color-card-border)] bg-[var(--color-background)]/90 backdrop-blur-sm px-4 sm:px-6 h-12 flex items-center justify-between gap-4">
      <div className="flex items-center gap-4 min-w-0">
        <SiteLogo showText={false} />
        <div className="h-5 w-px bg-[var(--color-card-border)] hidden sm:block" />
        <h1 className="text-sm font-medium truncate text-[var(--color-foreground)]/90">
          {title ?? "Editor"}
        </h1>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        {isLive && (
          <span className="text-[10px] sm:text-xs text-red-400 flex items-center gap-1.5">
